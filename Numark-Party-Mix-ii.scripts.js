...
// SETTINGS
NumarkPartyMix.jogScratchSensitivity = 340;  //340 lo cambio por 600
NumarkPartyMix.jogScratchAlpha = 1 / 8; // do NOT set to 2 or higher
NumarkPartyMix.jogScratchBeta = 1 / 8 / 32;
NumarkPartyMix.jogPitchSensitivity = 2;   // valor 2
NumarkPartyMix.jogSearchSensitivity = 1;  // valor 1
...
NumarkPartyMix.Deck = function (deckNumber) {
    ...
    this.wheelTurn = new components.Encoder({
        input: function (channel, _control, value, _status, group) {
            ...
            if (engine.isScratching(deckNumber)) {
                // Only scratch if touch was enabled
                engine.scratchTick(deckNumber, newValue);
            } else {
                // Otherwise jog/pitch bend as usual
                if (engine.getValue(group, "play") > 0) {
                    engine.setValue(group, "jog", newValue / NumarkPartyMix.jogPitchSensitivity);
                } else {
                    engine.setValue(group, "jog", newValue / NumarkPartyMix.jogSearchSensitivity);
                }
            }
        }
    });
    ...
    this.wheelTouch = new components.Button({
        ...
        input: function (_channel, _control, value, _status) {
            if (value > 0) {
                // Only enable scratch if not already enabled
                if (!engine.isScratching(deckNumber)) {
                    engine.scratchEnable(deckNumber,
                        NumarkPartyMix.jogScratchSensitivity,
                        33 + 1 / 3,
                        NumarkPartyMix.jogScratchAlpha,
                        NumarkPartyMix.jogScratchBeta,
                        true);
                }
            }
        }
    ...
    this.wheelUnTouch = new components.Button({
        midi: [0x80 + channel, 0x06],
        input: function (_channel, _control, value, _status) {
